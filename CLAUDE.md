...
- **Hardcoded vars** (set unconditionally via `--setenv`, also under `--keep-env`, override host values): all `=1` except where noted — `DO_NOT_TRACK`, `DISABLE_TELEMETRY`, `DISABLE_AUTOUPDATER`, `DISABLE_ERROR_REPORTING`, `DISABLE_BUG_COMMAND`, `CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY`, `DISABLE_INSTALL_GITHUB_APP_COMMAND`, `CLAUDE_DISABLE_CONFIG_WATCH`, **`CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=0`** (the only `=0` — `=1` triggers permission-mode hardening that overrides `--dangerously-skip-permissions` which blaude always passes; symptom: `Permission mode forced to default — CLAUDE_CODE_SUBPROCESS_ENV_SCRUB is set …`). Never add these to passthrough. Stored as `KEY=VALUE` pairs in `_hardcoded_env_vars`.
