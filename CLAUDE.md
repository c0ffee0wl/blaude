...
- **`--git` mounts the GPG agent socket dir** via `gpgconf --list-dirs socketdir` — needed for `commit.gpgsign=true`. `GPG_TTY` and `GNUPGHOME` are also auto-passed when set. Decoupled from `--ssh` because GPG-signed commits work without SSH (e.g. with `gh` over HTTPS).
