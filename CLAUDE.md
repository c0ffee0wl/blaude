...
- **Bypass-permissions blocker detection** (`_warn_if_bypass_disabled`): scans `$config_dir/settings.json`, `~/.claude/settings.json`, and `/etc/claude-code/managed-settings.json` for `permissions.disableBypassPermissionsMode: "disable"`. If found, prints a warning at startup — that key blocks `--dangerously-skip-permissions` (which blaude always passes), and we cannot override managed (enterprise) settings. **Important**: the regex check uses `$(< file)` *without* `2>/dev/null`; combining the bash file-read optimization with a redirect silently returns empty (subshell becomes a no-op). Existence + readability checks are sufficient before the read.
