...
- **Hardcoded vars** (set unconditionally via `--setenv`, also under `--keep-env`, override host values): `DO_NOT_TRACK=1`, `DISABLE_TELEMETRY=1`, `DISABLE_AUTOUPDATER=1`, `DISABLE_ERROR_REPORTING=1`, `DISABLE_BUG_COMMAND=1`, `CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY=1`, `DISABLE_INSTALL_GITHUB_APP_COMMAND=1`, `CLAUDE_DISABLE_CONFIG_WATCH=1`, `DISABLE_GROWTHBOOK=1`. Stored as `KEY=VALUE` pairs in `_hardcoded_env_vars`. Never add these to passthrough.
- **Hardcoded unsets** (forced absent via `--unsetenv`, also under `--keep-env`): `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB`. The upstream hardening predicate fires whenever the variable is *defined* (it overrides `--dangerously-skip-permissions`, symptom: `Permission mode forced to default — CLAUDE_CODE_SUBPROCESS_ENV_SCRUB is set …`), and `=0` does not suppress the banner despite the advice embedded in it — so blaude matches the documented default of "unset = disabled" instead of zeroing. Stored in `_hardcoded_unsetenv_vars`; also scrubbed from the passthrough whitelist alongside `_hardcoded_env_vars`.
