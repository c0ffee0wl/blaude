...
| Category | Variables |
|----------|-----------|
| **Authentication** | `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, `ANTHROPIC_BETAS`, `ANTHROPIC_CUSTOM_HEADERS`, `ANTHROPIC_FOUNDRY_*`, `ANTHROPIC_BEDROCK_BASE_URL`, `ANTHROPIC_BEDROCK_MANTLE_BASE_URL`, `ANTHROPIC_BEDROCK_SERVICE_TIER`, `ANTHROPIC_VERTEX_BASE_URL`, `AWS_BEARER_TOKEN_BEDROCK`, `CLAUDE_CODE_OAUTH_*` |
...
| **MCP** | `MCP_TIMEOUT`, `MCP_TOOL_TIMEOUT`, `MCP_OAUTH_CALLBACK_PORT`, `MCP_CLIENT_SECRET`, `MCP_SSE_URL`, `ENABLE_TOOL_SEARCH`, `ENABLE_CLAUDEAI_MCP_SERVERS`, `MCP_CONNECTION_NONBLOCKING`, `MCP_SERVER_CONNECTION_BATCH_SIZE`, `MCP_REMOTE_SERVER_CONNECTION_BATCH_SIZE`, `CLAUDE_CODE_MCP_SERVER_NAME`, `CLAUDE_CODE_MCP_SERVER_URL`, `CLAUDE_CODE_MCP_ALLOWLIST_ENV` |
| **Telemetry** | `CLAUDE_CODE_ENABLE_TELEMETRY`, `CLAUDE_CODE_OTEL_*`, `OTEL_*` (metrics, logs, exporter config, `OTEL_LOG_RAW_API_BODIES`), `DISABLE_COST_WARNINGS` |
| **UI/Display** | `CLAUDE_CODE_HIDE_ACCOUNT_INFO`, `CLAUDE_CODE_HIDE_CWD`, `CLAUDE_CODE_DISABLE_TERMINAL_TITLE`, `CLAUDE_CODE_DISABLE_MOUSE`, `CLAUDE_CODE_DISABLE_VIRTUAL_SCROLL`, `CLAUDE_CODE_NO_FLICKER`, `CLAUDE_CODE_SCROLL_SPEED`, `CLAUDE_CODE_ACCESSIBILITY`, `CLAUDE_CODE_SYNTAX_HIGHLIGHT`, `CLAUDE_CODE_ATTRIBUTION_HEADER`, `CLAUDE_CODE_TMUX_TRUECOLOR`, `CLAUDE_CODE_SKIP_PROMPT_HISTORY`, `CLAUDE_CODE_AUTO_CONNECT_IDE`, `FORCE_HYPERLINK`, `IS_DEMO` |
...
| **Updates/Installation** | `FORCE_AUTOUPDATE_PLUGINS`, `CLAUDE_CODE_PLUGIN_KEEP_MARKETPLACE_ON_FAILURE`, `CLAUDE_CODE_PLUGIN_GIT_TIMEOUT_MS`, `DISABLE_INSTALLATION_CHECKS`, `DISABLE_UPGRADE_COMMAND`, `DISABLE_DOCTOR_COMMAND`, `DISABLE_UPDATES`, `CLAUDE_CODE_SYNC_PLUGIN_INSTALL*` |
