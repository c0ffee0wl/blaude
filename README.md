...
| **Network/TLS** | `HTTP_PROXY`, `HTTPS_PROXY`, `SOCKS_PROXY`, `NO_PROXY` and the lowercase `https_proxy`, `http_proxy`, `socks_proxy`, `no_proxy` (Claude Code takes the first of `https_proxy`, `HTTPS_PROXY`, `http_proxy`, `HTTP_PROXY` that is set, so the lowercase spelling wins), `NODE_EXTRA_CA_CERTS`, `NODE_TLS_REJECT_UNAUTHORIZED` (CA-store selection is now `CLAUDE_CODE_CERT_STORE`, covered by the `CLAUDE_` prefix) |
